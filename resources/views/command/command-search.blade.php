...
<div class="container-fluid">
        ...
        <div class="col-md-12">
            <div class="card card-primary mx-auto">
                <div class="card-header">
                    <h3 class="card-title">ค้นหาข้อมูลทะเบียนคำสั่ง</h3>
                </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-primary mx-auto">
                <div class="card-header">
                    <h3 class="card-title">แสดงรายการทะเบียนคำสั่ง</h3>
                </div>
                <div class="container-fluid mx-auto mt-2 mb-2">
                    <div align="right">
                        <a href="{{ route('form.command-add') }}" title="send add">
                            <button type="button" class="btn btn-default" data-toggle="modal"
                                data-target="#modal-default">
                                <i class="fa fa-plus-circle"></i>
                                สร้างทะเบียนคำสั่งใหม่
                            </button>
                        </a>
                    </div>
                    <form method="post">
                    ...
                    <div class="row mx-auto">
                        <table id="tableCommand" class="table table-bordered table-striped w-100">
                            <thead>
                                <tr>
                                    <th>
                                        ลำดับ
...
<script lang="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var table = $('#tableCommand').DataTable({
            processing: true,
            serverSide: true,
            retrieve: true,
            orderCellsTop: true,
            fixedHeader: true,
            ajax: "{{ url('api/commandcontent') }}",
            columns: [{
        ...
        $('#tableCommand tbody').on('click', 'td.details-control', function () {
        ...
        $('#tableCommand tbody').on('click', 'td.data-control .btn-delete', function (e) {
            e.preventDefault();
            var tr = $(this).closest('tr');
            var row = table.row(tr);
            ...
            if (document != null) {
                swal("ต้องการลบข้อมูลคำสั่งนี้ใช่หรือไม่?", "เรื่อง " + document.title, {
                    dangerMode: true,
                    buttons: {
                        cancel: "ยกเลิก",
                        confirm: "ยืนยัน",
                    ...
                    if (val) {
                        ...
                        $.ajax("/deleteFileAndData/" + document.id, {
                            ...
                            success: function (data, status, xhr) {
                                swal("รายงานผล", "ลบข้อมูลคำสั่งสำเร็จแล้ว",
                                    "success");
        ...
        $('#tableCommand tbody').on('click', 'td.data-control .btn-edit', function (e) {
            e.preventDefault();
            var tr = $(this).closest('tr');
            var row = table.row(tr);
            var document = table.row(row).data();

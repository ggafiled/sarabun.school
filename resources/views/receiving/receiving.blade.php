        ...
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">ทะเบียนคุมหนังสือรับ</h3>
                    @if($editmode)
                    <a class="float-right" href="{{ route('form.receiving-add') }}" title="send add">
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label class="w3-text-blue">เลขทะเบียนรับ :</label>
...
<script>
    $(document).ready(function () {
        ...
        function getRunNo() {
            $.ajax("{{URL::to('/')}}/api/getReceivingLastId", {
                type: 'POST',
                processData: false,
                contentType: false,
    ...
    Dropzone.options.myDropzone = {
        ...
        init: function () {
            ...
            this.on("success", function (file, responseText) {
                var today = new Date().toISOString().split('T')[0];
                Swal.fire("รายงานผล", "บันทึกคุมทะเบียนรับสำเร็จแล้ว", "success").then(function () {
                    $("#id").val('');
                    $("#document_no").val('');
                    $("#source").val('');
                    $("#destination").val('');
